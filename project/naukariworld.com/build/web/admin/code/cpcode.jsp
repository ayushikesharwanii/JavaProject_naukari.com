...
<%
    ...
    String cmd="update tbllogin set pass='"+npass+"' where pass='"+opass+"'";
    boolean b=db.ExecuteUpdate(cmd);
if(cpass.equals(npass))
{
    if(b==true)
        out.print("<script>alert('password  changed successfully..');"+"window.location.href='../changepass.jsp';</script>");
    else
        out.print("<script>alert('old password is not matched');"+"window.location.href='../changepass.jsp';</script>");
}
else
{
   out.print("<script>alert('confirm password is not matched');"+"window.location.href='../changepass.jsp';</script>");  
}
%>